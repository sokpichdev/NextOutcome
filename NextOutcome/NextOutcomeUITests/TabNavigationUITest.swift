...
final class TabNavigationUITests: XCTestCase {
    ...
    /// TC-010: cycle through the three primary tabs plus the standalone Search
    /// button and verify each root's anchor UI.
    @MainActor
    func testAllTabsRender() throws {
        ...
        // 2. Breaking — biggest 24h movers; rows expose a
        ...
        attachScreenshot(of: app, named: "Tab 2 — Breaking")

        // 3. Portfolio — dynamic balance label, so select by index; the screen
        ...
        attachScreenshot(of: app, named: "Tab 3 — Portfolio")

        // 4. Search — the standalone button set apart from the three above.
        app.searchTab.tap()
        assertAppears(app.staticTexts["Search NextOutcome markets"], timeout: UIWait.load,
                      "Search should show its idle prompt before any query")
        attachScreenshot(of: app, named: "Search — standalone button")

        // And back Home without a reload flash. The rail's first chip is the unfiltered
        // feed, labelled "All" by Gamma's `top-navbar` row (and by `HubTab.fallbackNav`
        // before the fetch lands) — not "Trending".
        app.homeTab.tap()
        assertAppears(app.buttons["All"], timeout: UIWait.ui,
